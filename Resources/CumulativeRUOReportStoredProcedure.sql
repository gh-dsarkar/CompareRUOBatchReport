...
WITH ALL_SAMPLES AS (
               SELECT vv."GHRequestID",
               vv."SampleID" AS "SampleID",
               vv."RunID" AS "RunID",
               vv."Sample_status"
               FROM V_RESEARCH_REQUEST_INFO vv
               WHERE vv."ProjectId" = 'G360_3.0_IUO_E2E'
               AND vv."RequestStatus" = 'Released' and vv."RecordStatus" = 'Activating'
           ),
           --ACTIVE_SAMPLES CTE pulls in all samples that are not cancelled. This is so we can pull variant data strictly for these samples and not cancelled samples.
           ACTIVESAMPLES AS (
               SELECT ALL_SAMPLES."GHRequestID",
               ALL_SAMPLES."SampleID" AS "SampleID",
               ALL_SAMPLES."RunID" AS "RunID"
               FROM ALL_SAMPLES
               WHERE ALL_SAMPLES."Sample_status" != 'FAIL'
           ),
           RUOREPORTMODULES AS (
               SELECT DISTINCT ruorm.LIMSTABLENAME, ruorm.FIELDNAMES, ruorm.FIELDVALUES, ruorm.VARIANTTYPE
               FROM u_ghruoreportmodule ruorm
               WHERE ruorm.reportmoduleversion = 'v1.1.1' AND (exists (select ruoreportmodule from u_ruomoduleassignment
               where s_projectid = 'G360_3.0_IUO_E2E' and ruorm.reportmodule = ruoreportmodule) OR 'base' = ruorm.reportmodule)
           ...
           DELETION AS (
                   SELECT 'U_GHDELETION' as LIMSTABLENAME,
                       ...
                       NULL as "Molecular_consequence",
                       CASE cnv_type
                           WHEN 'loh' THEN 'loh_deletion'
                           WHEN 'homdel' THEN 'homozygous_deletion'
                           WHEN 'cov_del' THEN 'deletion' END as "CNV_type",
                       NULL as "COSMIC",
                       NULL as "dbSNP",
                       NULL as "ClinVar",
                       NULL as "ClinVarID",
           ...
           INDEL AS (
                   SELECT  'U_GHINDEL' as LIMSTABLENAME,
                       ...
                       CONCAT(CONCAT('="',(CASE WHEN exon > 0 then TO_CHAR(exon) END)), '"') as "Exon",
                       (CASE WHEN UPPER(SOMATIC_CALL) = UPPER('somatic') THEN 1 ELSE 0 END) as "Alt-NoAlt",
                       mut_nt as "Mut_nt",
           ...
           SELECT
               ...
               vv."Physician_name"
           FROM V_RESEARCH_REQUEST_INFO vv
                    left join VARIANT v on v.SAMPLE_ID=vv."SampleID" and v.RUN_ID= vv."RunID"
           ...
           where exists (SELECT vv."SampleID" from ALL_SAMPLES where vv."SampleID" = ALL_SAMPLES."SampleID")
